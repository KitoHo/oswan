...
{
    WORD *pSBuf;            // ƒf[ƒ^‘‚«ž‚Ýƒoƒbƒtƒ@
    WORD *pSWrBuf;          // ª‚Ì‘‚«ž‚ÝˆÊ’u—pƒ|ƒCƒ“ƒ^
    int *pZ;                // «‚ÌƒCƒ“ƒNƒŠƒƒ“ƒg—pƒ|ƒCƒ“ƒ^
    int ZBuf[0x100];        // FGƒŒƒCƒ„[‚Ì”ñ“§–¾•”‚ð•Û‘¶
    int *pW;                // «‚ÌƒCƒ“ƒNƒŠƒƒ“ƒg—pƒ|ƒCƒ“ƒ^
    int WBuf[0x100];        // FGƒŒƒCƒ„[‚ÌƒEƒBƒ“ƒh[‚ð•Û‘¶
    int OffsetX;            // 
    int OffsetY;            // 
    BYTE *pbTMap;           // 
    int TMap;               // 
...
/*********************************************************************/
    ...
    if((DSPCTL & 0x02) && Layer[1])          //FG layer•\Ž¦
    {
        if((DSPCTL & 0x30) == 0x20) // ƒEƒBƒ“ƒhƒE“à•”‚Ì‚Ý‚É•\Ž¦
        {
        ...
        else if((DSPCTL & 0x30) == 0x30) // ƒEƒBƒ“ƒhƒEŠO•”‚Ì‚Ý‚É•\Ž¦
        {
            for(i = 0, pW = WBuf + 8; i < 224; i++)
            {
                *pW++ = 0;
